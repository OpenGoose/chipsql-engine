...
import { Value, ValueTypes } from "../../../chips-lq/types/values/value.type";
...
export class MssqlPartsCompiler<T extends Object> implements IQueryPartsCompiler<T> {
    ...
    value = (value: Value<T>) => {
        const processValue = () => {
            switch (value.valueType) {
                case ValueTypes.RAW_SQL: return value.sql;
                case ValueTypes.RAW_VALUE: return this.escape(value.value);
                case ValueTypes.ALL_COLUMNS: return '*';
                case ValueTypes.COLUMN: return joinParts([value.tableAlias ? `[${value.tableAlias}]` : null, `[${value.field}]`], '.');
            }
            throw new UnavailableFeatureError(value.valueType);
        }

        return joinParts([value.distinct ? 'DISTINCT' : null, processValue(), value.alias ? `AS '${value.alias}'` : null]);
    }

    table = (table: Table<T>) => {
        return joinParts([
          joinParts([
            table.schema ? `[${table.schema}]` : null,
            `[${table.name}]`,
          ], '.'),
    ...
    escape = (value: AllowedValues) => {
        if (isDate(value)) return format(value as Date, "YYYY-MM-DD hh:mm:ss[.nnnnnnn]");
        switch (typeof value) {
            case 'string':
